...
void emOrdem_ArvAVL(ArvAVL *raiz){
    ...
    if(*raiz != NULL){
        ...
        //printf("%d\n",(*raiz)->info);
        printf("Drone %.2f: altura(%d) fator de balanceamento(%d)\n",(*raiz)->info.peso,altura_NO(*raiz),fatorBalanceamento_NO(*raiz));
...
int insere_ArvAVL(ArvAVL *raiz, struct Drone valor){
    int res;
    ...
    if(valor.peso < atual->info.peso){
        if((res = insere_ArvAVL(&(atual->esq), valor)) == 1){
            if(fatorBalanceamento_NO(atual) >= 2){
                if(valor.peso < (*raiz)->esq->info.peso ){
                    RotacaoLL(raiz);
                }else{
                    RotacaoLR(raiz);
                }
        ...
        if(valor.peso > atual->info.peso){
            if((res = insere_ArvAVL(&(atual->dir), valor)) == 1){
                if(fatorBalanceamento_NO(atual) >= 2){
                    if((*raiz)->dir->info.peso < valor.peso){
                        RotacaoRR(raiz);
                    }else{
                        RotacaoRL(raiz);
                    }
...
int imprime_arvore(ArvAVL *a, int count) {
 if(a) {
